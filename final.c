    ...
    *tamanho = 0;
    ...
    while (fgets(buffer, sizeof(buffer), arquivo))
    {
        if (sscanf(buffer, "Endereco completo: %[^\n]", buffer) == 1)
        {
            ...
            if (enderecos == NULL)
            {
                printf("Erro: Falha ao alocar memória para endereços.\n");
                fclose(arquivo);
                return NULL;
            }
...
void AdicionarDadosNoArquivo(void **registro, int *num, TipoRegistro tipo)
{
    for (int i = 0; i < *num; i++)
    {
        ...
        case ENTREGAREALIZADA:
        {
            ...
            fprintf(arquivoEntregasRealizadas, "ID do funcionário: %d\n", e.idfuncionario);
            fprintf(arquivoEntregasRealizadas, "ID do veículo: %d\n", e.idveiculo);
            fprintf(arquivoEntregasRealizadas, "Origem: %s\n", e.origemEntrega);
            fprintf(arquivoEntregasRealizadas, "Destino: %s\n", e.destinoEntrega);
            fprintf(arquivoEntregasRealizadas, "Tempo: %d\n", e.tempoEntrega);
...
void setorentrega()
{
    ...
    {
        system("cls");
        printf("===== Menu de Operacões - Entregas =====\n \n Adicionar -> 1 \n Visualizar -> 2 \n Alterar -> 3 \n Deletar -> 4 \n Realizar Entrega -> 5 \n Voltar Para Menu de Setores -> 6 \n Sair do programa -> 7 \n\nQual operacao deseja realizar: ");
        scanf("%d", &numTipoDeOperacao);
        getchar();
        ...
        case 5:
            printf("\nQuantas entregas você deseja realizar: ");
            scanf("%d", &numdeentregas);
            getchar();
